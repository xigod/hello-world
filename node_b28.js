...
function getFileList(srcFolder, destFolder) {
    if (!gfileList || gfileList.length === 0) {
        ...
        files.files.forEach(function(val) {
            //console.log("gfileList.length="+val)
...
function getLineFileList(srcFolder, destFolder) {
    if (!gfileList || gfileList.length === 0) {
        srcFolder = correctPath(srcFolder);
        ...
        files.files.forEach(function(val) {
            ...
            //console.log("file.relative(CONFIG.src, val)========="+file.relative(CONFIG.src, val));
            //console.log("destFolder========="+destFolder);
            if ('/' + file.relative(CONFIG.src, val)) {
                gfileList.push({
                    fileName: val,
                    fileType: path.extname(val)
                });
...
function getLangData(srcdir) { //提取入口
    var langFetchArr = [];
    ...
    writeFile(saveTo+'.xlsx', langFetchArr);
    writeExcel(path.join(path.dirname(saveTo), 'remark.xlsx'), unique(B.getRemark(), 1));
}

//获取文件夹代码行数
function getLine(srcdir) {
    var count = 0;
    if (srcdir && typeof srcdir == 'string' && fs.lstatSync(srcdir).isDirectory()) {
        gfileList = getLineFileList(srcdir);
        console.log("count======12345"+gfileList.length);
        gfileList.forEach(function(val) {
            console.log("文件为++++++++++++++++"+val.fileName);
            console.log("行数为++++++++++++++++"+doGetLine(val));
            count += doGetLine(val);
            //console.log("内容++++++++++++++++"+langFetchArr);
        });
...
//返回一个文件代码行数
function doGetLine(file) {
    //console.log("count======123"+count);
    var content = fs.readFileSync(file.fileName, 'utf-8'),
        count = 1;
...
if(args.getLine) {//统计代码行数
    CONFIG.src = correctPath(args.src);
    console.log("代码总量为：" + getLine(CONFIG.src) + "行！");
} else if (!args.h) {//翻译工具
